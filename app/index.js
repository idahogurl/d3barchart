...
var BarChart = (function (_super) {
    ...
    function BarChart() {
        ...
        _this.margin = new Margin(30, 30, 40, 50);
        _this.height = 400 - _this.margin.top - _this.margin.bottom;
        _this.width = 600 - _this.margin.left - _this.margin.right;
        _this.axisLabel = "Gross Domestic Product, USA";
        _this.title = "Gross Domestic Product";
        var barWidth = 50, barOffset = 5;
        _this.barData = [];
        _this.fetchData();
        return _this;
    }
    BarChart.prototype.createChart = function () {
        ...
        var yScale = D3.scaleLinear()
            ...
            .range([0, this.height]);
        debugger;
        var years = this.barData.map(function (m) { return m[0].getFullYear(); });
        var maxYear = D3.max(years);
        var minYear = D3.min(years);
        var xScale = D3.scaleBand()
            .domain(D3.range(minYear, maxYear).map(function (m) { return m.toString(); }))
    ...
    BarChart.prototype.addHorizontalAxis = function () {
        ...
        var height = this.height;
        var margin = this.margin;
        var hAxis = D3.axisBottom(this.xScale)
            .tickValues(this.xScale.domain()
            .filter(function (d, i) {
            return !(i % (_this.barData.length / 10));
        }));
        var hGuide = D3.select("svg").append("g").call(hAxis);
    ...
    BarChart.prototype.addVerticalAxis = function () {
        ...
        var vAxis = D3.axisLeft(vGuideScale)
            .ticks(10);
        var vGuide = D3.select("svg").append("g").call(vAxis);
